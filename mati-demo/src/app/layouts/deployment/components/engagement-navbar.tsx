...
import { data } from "@/components/nav-bars/nav-data"
import {
    Bell,
    BookOpen,
    ChevronsUpDown,
...
import { NavMain } from "@/components/nav-bars/components/nav-main"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { cn } from "@/lib/utils"

export function EngagementNavbar({ children }: { children?: React.ReactNode }) {
    return (
        <React.Suspense fallback={<div className="min-h-screen bg-gray-50" />}>
            <EngagementNavbarContent>{children}</EngagementNavbarContent>
        </React.Suspense>
    )
}

function EngagementNavbarContent({ children }: { children?: React.ReactNode }) {
    // State for context switchers - kept for interactivity but defaulted
    ...
    // Fixed Top Item as per user request
    const activeTopItem = "Engagement"

    // Filter items for "Verifier" role as requested in the URL example
    const selectedRoles = ["Verifier"]
    ...
    // Ensure "Farm App" is expanded and "Engagement" is active in sidebar logic
    // We update the data slightly for this view
    const sidebarNavMain = filteredNavMain.map(item => {
        if (item.title === "Farm App") {
            return {
                ...item,
                isActive: true, // Expand Farm App
                items: item.items?.map(subItem => ({
                    ...subItem,
                    isActive: subItem.title === "Engagement" // Highlight Engagement
                }))
            }
        }
        return item
                            ...
                            <NavigationMenuList>
                                {customTabs.map((tab) => (
                                    <NavigationMenuItem key={tab.title}>
                                        <NavigationMenuTrigger
                                            className={cn(
                                                "h-10 bg-transparent hover:bg-transparent data-[state=open]:bg-transparent data-[active]:bg-transparent px-2 text-sm font-medium transition-colors cursor-default",
                                                activeTopItem === tab.title ? "text-black" : "text-gray-400",
                                        ...
                                        {/* Dropdown content only for Engagement for consistency if needed, 
                                            or just disable all triggers if it's meant to be unchangeable */}
                                            ...
                                            <ul className={cn(
                                                ...
                                                <li className={cn(tab.items && tab.items.length > 0 ? "row-span-2" : "")}>
                                                    <NavigationMenuLink asChild>
                                                        <div className="flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-gray-50 to-gray-100 p-4 no-underline outline-none border border-gray-100">
                                                    ...
                                                    <ListItem key={item.title} title={item.title}>
                                                        {item.statuses.length > 0 ? (
                                                            <div className="flex flex-wrap gap-2 mt-2">
                                                                {item.statuses.map(s => (
                                                                    <div key={s} className="h-5 text-[10px] px-2 bg-white border border-gray-200 text-gray-600 rounded flex items-center">
                                                                        {s}
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        ) : (
                                                            <span className="text-xs text-gray-400">Section details</span>
                                                        )}
                                                    </ListItem>
                                                ))}
                                            </ul>
