...
function Calendar({
  ...
  showTime?: boolean
  timeValue?: string // format "hh:mm AM/PM"
  ...
  React.useEffect(() => {
    if (timeValue) {
      ...
      if (match) {
        setHours(match[1])
        setMinutes(parseInt(match[2]).toString())
        setAmpm(match[3].toUpperCase())
      }
    }
  }, [timeValue])

  const handleTimeUpdate = (newHours: string, newMinutes: string, newAmpm: string) => {
    setHours(newHours)
    ...
    <div className="flex flex-col gap-0">
      <DayPicker
        ...
        classNames={{
          ...
          button_previous: cn(
            ...
            "h-[--cell-size] w-[--cell-size] select-none p-0 aria-disabled:opacity-50 rounded-md",
      ...
      {showTime && (
        <div className="flex items-center justify-center gap-2 p-3 border-t bg-background">
          ...
          <Select value={ampm} onValueChange={(v) => handleTimeUpdate(hours, minutes, v)}>
            <SelectTrigger className="w-[64px] h-8 text-xs">
